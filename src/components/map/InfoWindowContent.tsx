...
export default function InfoWindowContent({
  ...
  return (
    <div className="w-64 bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 max-w-none">
      ...
      <div className="relative">
        {firstPhoto ? (
          ...
          <div className="h-12 bg-gradient-to-r from-[#FF6B6B]/20 to-[#EE8C80]/20" />
        )}
        ...
        <div className="absolute top-1.5 right-1.5">
          <button 
            ...
            title="Save place"
          >
            <Heart className="h-3 w-3 text-[#FF6B6B] hover:fill-current transition-colors" />
          </button>
        </div>
      </div>
      ...
      <div className="p-2.5">
        ...
        <div className="mb-2">
          <div className="flex items-start justify-between mb-1">
            <h3 className="text-sm font-semibold text-gray-900 leading-tight pr-2 line-clamp-1">{place.name}</h3>
          ...
          <div className="flex items-center justify-between text-xs">
            {place.category && (
              <span className="text-gray-600 truncate">{place.category}</span>
            )}
            <span className="text-xs font-medium text-[#FF6B6B] ml-1">{priceIndicator}</span>
          </div>
        </div>
            ...
            <div className="flex justify-between text-xs">
              {place.avgValue && (
                <div className="flex flex-col items-center">
                  ...
                  <span className="font-bold text-[#FF6B6B] text-xs">{place.avgValue.toFixed(1)}</span>
                </div>
              )}
              {place.avgAtmosphere && (
                <div className="flex flex-col items-center">
                  <span className="text-gray-500 text-xs">Vibe</span>
                  <span className="font-bold text-[#FF6B6B] text-xs">{place.avgAtmosphere.toFixed(1)}</span>
                </div>
              )}
              <div className="flex flex-col items-center">
                <span className="text-gray-500 text-xs">Reviews</span>
                <span className="font-bold text-[#FF6B6B] text-xs">{place.reviewers.length}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {onViewRestaurant && (
          <Button
            ...
            className="w-full bg-[#FF6B6B] hover:bg-[#FF6B6B]/90 text-white text-xs py-1.5 h-auto rounded-md transition-colors"
          >
